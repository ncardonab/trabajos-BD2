...
m NUMBER;
BEGIN
    SELECT COUNT(*) INTO i FROM indexdepskip;
    i := i-1;
    ...
    WHILE i > 0 LOOP
        FOR nodo IN (SELECT * FROM indexdepskip WHERE numnodo = i) LOOP
            ...
            FOR ptr IN (SELECT t2.numnodo num, COUNT(t2.numnodo) cant FROM indexdepskip t, TABLE(t.grupoDePunteros) t2 WHERE t.numnodo = i GROUP BY t2.numnodo) LOOP
                mi_array(ptr.num) := ptr.cant;
            END LOOP;
            SELECT COUNT(*) total INTO l FROM indexdepskip t, TABLE(t.grupoDePunteros) t2 WHERE t.numnodo = i;
            j := mi_array.LAST;
            WHILE l >= 1 LOOP
                m := mi_array(j);
                FOR k IN 1 .. m LOOP
            ...
            SELECT ptrback INTO i FROM indexdepskip WHERE numnodo = i;
        END LOOP;
